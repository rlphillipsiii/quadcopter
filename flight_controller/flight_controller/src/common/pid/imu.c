...
#define XM_WRITE(reg, length)   serial_write(XM, reg, &g_data[0], length);
#define GYRO_WRITE(reg, length) serial_write(GYRO, reg, &g_data[0], length);

#define XM_READ(reg, length)    serial_read(XM, reg, &g_data[0], length);
#define GYRO_READ(reg, length)  serial_read(GYRO, reg, &g_data[0], length);
...
static uint8_t g_data[BUFFER_SIZE];
